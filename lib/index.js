...
async function probe(ctx, body) {
  ...
  if (api !== 'openai-completions' && api !== 'openai-responses') {
    throw new Error('当前仅支持 OpenAI 兼容的 Completions 或 Responses 中转站')
  }
  ...
  if (!response.ok) {
    const reason = response.status === 401 || response.status === 403
      ? (apiKey ? '，API Key 被中转站拒绝，请确认 Key、权限和 /models 接口权限' : '，请在卡片中输入一次性 API Key')
      : ''
...
export function apply(ctx) {
  const disposer = ctx.webServer.register({
    ...
    handler: async (req, res) => {
      let url
      try { url = new URL(req.url, 'http://127.0.0.1') } catch { return sendJson(res, 400, { ok: false, error: '请求地址无效' }) }
      if (req.headers['x-dsh-reasoning'] !== '1') return sendJson(res, 403, { ok: false, error: '请求被拒绝' })
      if (url.pathname !== `${API}/probe`) return sendJson(res, 404, { ok: false, error: '接口不存在' })
      if (req.method !== 'POST') return sendJson(res, 405, { ok: false, error: '请求方法不支持' })
