...
async function probe(ctx, body) {
  ...
  const apiKey = await resolveCredential(ctx, body.apiKeyEnv)
  const response = await fetch(url, {
    headers: { accept: 'application/json', ...(apiKey ? { authorization: `Bearer ${apiKey}` } : {}) },
    signal: AbortSignal.timeout(15000),
  })
  if (!response.ok) {
    throw new Error(`${url} 返回 HTTP ${response.status}${response.status === 401 || response.status === 403 ? '，请检查 API Key' : ''}`)
  }
