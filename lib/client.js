window.__ModuleLoader__.load({ id: 'dsh-better-model-thinking-control', factory: (require) => {
  ...
  const LABELS = { off: '关闭 / Off', minimal: '最低 / Minimal', low: '低 / Low', medium: '中 / Medium', high: '高 / High', xhigh: '超高 / XHigh', max: '最大 / Max' }
  ...
  function Card(props) {
    ...
    if (state.status !== 'ready') {
      ...
      return props.standalone === true
        ? React.createElement('div', { className: 'dbmt-card dbmt-unavailable' },
            React.createElement('strong', null, '模型思考强度 / Model Thinking Effort'),
            React.createElement('p', null, state.status === 'loading'
              ? '正在读取 DSH 模型配置… / Loading DSH model configuration…'
              : '未检测到 DSH 的 llm-pi-ai 模型配置。请先在“模型”设置中启用或添加 OpenAI 兼容中转站，然后重新打开此页。 / No llm-pi-ai configuration was found. Add an OpenAI-compatible provider in Models first.'))
        : null
    }
    ...
    async function autoFetch(providerId) {
      ...
      try {
        ...
        if (!result.data || !result.data.ok) throw new Error(result.data && result.data.error || `HTTP ${result.status}`)
        ...
        setMessage(`已拉取 ${result.data.models.length} 个模型 / Fetched ${result.data.models.length} models; review and save`)
      } catch (error) {
        setMessage('自动拉取失败 / Auto fetch failed: ' + String(error && error.message || error))
    ...
    return React.createElement('div', { className: 'dbmt-card' },
      React.createElement('button', { className: 'dbmt-header', type: 'button', onClick: () => setOpen(!open), 'aria-expanded': open },
        React.createElement('span', null,
          React.createElement('strong', null, '模型思考强度 / Model Thinking Effort'),
          React.createElement('small', null, '按中转站和模型设置 / Configure reasoningEfforts per provider and model')), open ? '收起 / Collapse' : '展开 / Expand'),
      open ? React.createElement('div', { className: 'dbmt-body' },
        !state.writable ? React.createElement('div', { className: 'dbmt-warn' }, '当前 DSH 设置为只读 / DSH settings are read-only') : null,
        configuredProviders.length === 0 ? React.createElement('div', { className: 'dbmt-muted' }, '尚未配置 llm-pi-ai provider，请先到 DSH 的模型设置添加中转站。 / No llm-pi-ai provider is configured; add one in DSH Models.') : null,
        configuredProviders.map((providerId) => {
          ...
          return React.createElement('section', { className: 'dbmt-provider', key: providerId },
            React.createElement('button', { type: 'button', className: 'dbmt-provider-head dbmt-provider-toggle', onClick: () => setProviderOpen((current) => Object.assign({}, current, { [providerId]: !expanded })), 'aria-expanded': expanded },
              ...
              React.createElement('span', { className: 'dbmt-provider-state' }, expanded ? '收起 / Collapse' : '展开 / Expand')),
            expanded ? React.createElement('div', { className: 'dbmt-provider-body' },
              React.createElement('label', { className: 'dbmt-field' },
                React.createElement('span', null, '一次性 API Key（可选） / One-shot API key (optional)'),
                React.createElement('input', { type: 'password', autoComplete: 'off', value: probeKeys[providerId] || '', placeholder: '仅用于本次自动拉取，不会保存 / Used only for this fetch; never saved', onChange: (event) => setProbeKeys((current) => Object.assign({}, current, { [providerId]: event.target.value })) })),
              React.createElement('div', { className: 'dbmt-provider-actions' },
                React.createElement('button', { type: 'button', disabled: busy || !provider.baseURL, onClick: () => autoFetch(providerId) }, '自动拉取 / Auto Fetch')),
              models.map((model, index) => React.createElement('div', { className: 'dbmt-model', key: `${providerId}:${index}` },
                React.createElement('div', { className: 'dbmt-model-title' },
                  React.createElement('input', { value: model.id || '', placeholder: '模型 ID / Model ID', onChange: (event) => updateModel(providerId, index, { id: event.target.value }) }),
                  React.createElement('button', { type: 'button', title: '删除模型 / Remove model', onClick: () => removeModel(providerId, index) }, '删除 / Remove')),
                React.createElement('div', { className: 'dbmt-levels' },
                  LEVELS.map((level) => {
                    const efforts = asEfforts(model)
                    const checked = efforts !== false && Object.prototype.hasOwnProperty.call(efforts, level)
                    return React.createElement('label', { key: level, className: 'dbmt-level' },
                  ...
                  React.createElement('label', { className: 'dbmt-level dbmt-none' },
                    React.createElement('input', { type: 'checkbox', checked: asEfforts(model) === false, onChange: (event) => updateModel(providerId, index, { reasoningEfforts: event.target.checked ? false : { off: null } }) }), '非推理模型 / Non-reasoning')),
                React.createElement('div', { className: 'dbmt-hint' }, '自动识别到的档位会写入模型配置；未识别时可手动勾选。 / Detected levels are staged into the model; select them manually when unavailable.'))),
              React.createElement('button', { type: 'button', className: 'dbmt-add', onClick: () => addModel(providerId) }, '+ 添加模型 / Add model')) : null
          )
        }),
        ...
        React.createElement('div', { className: 'dbmt-actions' },
          React.createElement('button', { type: 'button', disabled: busy, onClick: reset }, '撤销未保存修改 / Discard changes'),
          React.createElement('button', { type: 'button', className: 'dbmt-primary', disabled: busy || !state.writable, onClick: save }, busy ? '处理中… / Working…' : '保存配置 / Save')))
      : null)
  }

  const CSS = `.dbmt-card{border:1px solid rgba(127,127,127,.28);border-radius:8px;margin:12px 0;background:rgba(127,127,127,.04);overflow:hidden}.dbmt-header{display:flex;justify-content:space-between;align-items:center;width:100%;padding:13px 14px;border:0;background:transparent;color:inherit;text-align:left;cursor:pointer}.dbmt-header span{display:grid;gap:3px}.dbmt-header small{font-size:11px;opacity:.65;font-weight:400}.dbmt-body{display:flex;flex-direction:column;gap:10px;padding:0 14px 14px}.dbmt-provider{border-top:1px solid rgba(127,127,127,.2);padding-top:10px}.dbmt-provider-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.dbmt-provider-head strong{font-size:13px}.dbmt-provider-head code{font-size:10px;opacity:.6;flex:1}.dbmt-provider-head button,.dbmt-actions button,.dbmt-add,.dbmt-model-title button{border:1px solid rgba(76,125,255,.5);border-radius:6px;background:transparent;color:#4c7dff;padding:5px 9px;cursor:pointer}.dbmt-provider-head button:disabled,.dbmt-actions button:disabled{opacity:.45;cursor:default}.dbmt-model{margin-top:8px;padding:9px;border:1px solid rgba(127,127,127,.22);border-radius:7px}.dbmt-model-title{display:flex;gap:6px}.dbmt-model-title input{flex:1;min-width:0}.dbmt-model-title input,.dbmt-model-title button{box-sizing:border-box;padding:6px 8px;border:1px solid rgba(127,127,127,.4);border-radius:6px;background:rgba(127,127,127,.06);color:inherit}.dbmt-levels{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.dbmt-level{display:inline-flex;gap:4px;align-items:center;padding:4px 7px;border-radius:5px;background:rgba(76,125,255,.08);font-size:11px;cursor:pointer}.dbmt-none{color:#c44}.dbmt-hint,.dbmt-muted{font-size:11px;opacity:.65}.dbmt-add{margin-top:8px}.dbmt-actions{display:flex;gap:8px;justify-content:flex-end;border-top:1px solid rgba(127,127,127,.2);padding-top:10px}.dbmt-primary{background:rgba(76,125,255,.12)!important}.dbmt-success{color:#2ea043;font-size:12px}.dbmt-error,.dbmt-warn{color:#c44;font-size:12px}`
  const EXTRA_CSS = `.dbmt-provider-toggle{width:100%;text-align:left;color:inherit;background:transparent;border:0;padding:7px 0;cursor:pointer}.dbmt-provider-state{font-size:11px;color:#4c7dff}.dbmt-provider-body{display:flex;flex-direction:column;gap:8px}.dbmt-field{display:flex;flex-direction:column;gap:5px;font-size:11px;font-weight:600}.dbmt-field input{box-sizing:border-box;width:100%;padding:7px 9px;border:1px solid rgba(127,127,127,.4);border-radius:6px;background:rgba(127,127,127,.06);color:inherit}.dbmt-provider-actions{display:flex;justify-content:flex-end}.dbmt-provider-actions button{border:1px solid rgba(76,125,255,.5);border-radius:6px;background:transparent;color:#4c7dff;padding:5px 9px;cursor:pointer}.dbmt-provider-actions button:disabled{opacity:.45;cursor:default}.dbmt-unavailable{display:grid;gap:8px;padding:16px}.dbmt-unavailable p{margin:0;font-size:12px;line-height:1.65;opacity:.72}`
  ...
  function apply(ctx) {
    ...
    const registerCard = () => {
      ...
      slots.inject('settings.plugin.item', () => slots.register({
        ...
        // Older DSH Web builds declare this as a list slot and require `id`.
        // Newer keyed builds dispatch by `key`; ship both fields so this
        // external plugin remains loadable across both supported variants.
        id: 'better-model-thinking-control',
        key: NS,
        inject: () => ({
      ...
      // A dedicated settings page is the compatibility fallback for DSH builds
      // whose Plugins configuration surface does not dispatch external cards.
      // This is a list slot on all currently released Web builds, hence `id`.
