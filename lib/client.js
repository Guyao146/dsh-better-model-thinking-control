window.__ModuleLoader__.load({ id: 'dsh-better-model-thinking-control', factory: (require) => {
  ...
  const LABELS = { off: '关闭', minimal: '最低', low: '低', medium: '中', high: '高', xhigh: '超高', max: '最大' }
  ...
  function Card(props) {
    ...
    React.useEffect(() => {
      if (state.status === 'ready') setProviders(copy(state.value.providers))
    ...
    if (state.status !== 'ready') {
      ...
      return props.standalone === true
        ? React.createElement('div', { className: 'dbmt-card dbmt-unavailable' },
            ...
            React.createElement('p', null, '未检测到 DSH 的 llm-pi-ai 模型配置。请先在“模型”设置中启用或添加 OpenAI 兼容中转站，然后重新打开此页。'))
        : null
    }
    ...
    function reset() {
      setProviders(copy(state.value.providers)); setMessage('')
    }
    ...
    async function autoFetch(providerId) {
      ...
      try {
        const result = await apiFetch(`${API}/probe`, {
          ...
          body: JSON.stringify({ provider: providerId, baseURL: provider.baseURL, api: provider.api, apiKeyEnv: provider.apiKeyEnv }),
        })
    ...
    return React.createElement('div', { className: 'dbmt-card' },
      React.createElement('button', { className: 'dbmt-header', type: 'button', onClick: () => setOpen(!open), 'aria-expanded': open },
        React.createElement('span', null,
          React.createElement('strong', null, '模型思考强度'),
          React.createElement('small', null, '按中转站和模型设置 DSH 的 reasoningEfforts')), open ? '收起' : '展开'),
      open ? React.createElement('div', { className: 'dbmt-body' },
        ...
        configuredProviders.length === 0 ? React.createElement('div', { className: 'dbmt-muted' }, '尚未配置 llm-pi-ai provider，请先到 DSH 的模型设置添加中转站。') : null,
        configuredProviders.map((providerId) => {
          const provider = providers[providerId] || {}
          const models = Array.isArray(provider.models) ? provider.models : []
          return React.createElement('section', { className: 'dbmt-provider', key: providerId },
            React.createElement('div', { className: 'dbmt-provider-head' },
            ...
            models.map((model, index) => React.createElement('div', { className: 'dbmt-model', key: `${providerId}:${index}` },
              ...
              React.createElement('div', { className: 'dbmt-levels' },
                ...
                React.createElement('label', { className: 'dbmt-level dbmt-none' },
                  React.createElement('input', { type: 'checkbox', checked: asEfforts(model) === false, onChange: (event) => updateModel(providerId, index, { reasoningEfforts: event.target.checked ? false : { off: null } }) }), '非推理模型'))),
              React.createElement('div', { className: 'dbmt-hint' }, '自动识别到的档位会写入模型配置；未识别时可手动勾选。')),
  ...
  function apply(ctx) {
    ...
    const store = {
      getSnapshot: () => {
        ...
        return Object.assign({}, snapshot, { value: snapshot.value || { providers: {} } })
      },
