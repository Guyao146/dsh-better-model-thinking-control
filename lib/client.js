  ...
  function Card(props) {
    ...
    function renderModel(providerId, model, index) {
      ...
      const effortSelect = React.createElement('div', { className: 'dbmt-effort-select' },
        ...
        React.createElement('label', { className: 'dbmt-level dbmt-none' },
          React.createElement('input', { type: 'checkbox', checked: efforts === false, onChange: (event) => updateModel(providerId, index, { reasoningEfforts: event.target.checked ? false : { off: null } }) }), '非推理模型'))
    ...
    const providerSections = configuredProviders.map((providerId) => {
      ...
      const providerBody = React.createElement('div', { className: 'dbmt-provider-body' },
        ...
        models.map((model, index) => renderModel(providerId, model, index)),
